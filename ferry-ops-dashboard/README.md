...
- **Safety Systems**: Fire alarm, bilge level, and CO2 monitoring with automatic alerts
- **Navigation Display**: Speed, heading, GPS position with compass visualization
- **MQTT Integration**: Direct connection to vessel MQTT broker for real-time telemetry
...
- **Alert System**: Real-time emergency notifications with severity levels and fire alarm alerts
- **Historical Analytics**: Full-featured time-series data visualization with Chart.js
- **Weather Integration**: Environmental conditions display
- **Data Persistence**: SQLite database for historical telemetry storage
...
### Historical Data
- **Time Ranges**: 1 hour, 6 hours, 12 hours, 24 hours views
- **Metrics Tracked**:
  - Engine: RPM, temperature, fuel flow
  - Power: Battery SOC, voltage, generator load
  - Navigation: Speed
  - Safety: Bilge level, CO2 level
- **Data Collection**: Every 5 seconds with automatic aggregation
- **Data Retention**: 1-day retention with 5-minute and hourly averages
- **Export**: CSV export functionality for all historical data
- **Visualization**: Interactive Chart.js charts with time-based X-axis
...
### Data Export
```
GET /api/historical/:vessel/export?range=24h
```
...
- `request_historical`: Request historical data for specific vessel/metric/range
- `acknowledge_emergency`: Acknowledge emergency alerts

### Server → Client
- `initial_data`: Initial dashboard state
- `vessel_update`: Real-time vessel telemetry
- `new_alert`: New alert notification including fire alarms
- `emergency_alert`: Critical emergency notification
- `weather_update`: Weather data update
- `historical_data`: Historical chart data response
...
```
ferry-ops-dashboard/
├── server.js              # Express server with MQTT integration
├── package.json           # Dependencies
├── fly.toml              # Fly.io configuration
├── Dockerfile            # Container configuration
├── db/                   # Database layer
│   ├── database.js       # SQLite database initialization
│   └── historical-data.js # Historical data service
├── workers/
│   └── data-collector.js # Background telemetry collector
└── public/               # Static assets
    ...
    └── js/
        ...
        └── charts.js     # Historical data charts with Chart.js
```
...
3. Deploy: `fly deploy --ha=false` (single instance for demo)
4. Set secrets: `fly secrets set KEY=value`

### Database
- SQLite database is automatically created in `/app/data/ferry_telemetry.db`
- Data directory is persisted in Docker container
