...
// Enhanced with safe debugging instrumentation
...
class OperationsDashboard {
  constructor() {
    ...
    // Debug instrumentation (safe execution)
    this.debugEnabled = window.location.search.includes('debug=true') || 
    ...
    // Safe debug logging
    this.safeDebugLog('🚢 BC Ferries Operations Dashboard initialized');
    console.log('🚢 BC Ferries Operations Dashboard initialized');
  }
  
  // Safe debug logging helper
  safeDebugLog(message, type = 'info', data = null) {
    try {
      if (this.debugEnabled && window.BCFDebug) {
        window.BCFDebug.log(message, type, data);
      }
    } catch (e) {
      // Silent fail to prevent breaking production
    }
  }
  ...
  updateWebSocketStatus(status) {
    ...
    // Safe debug logging
    this.safeDebugLog(`WebSocket Status Update: ${status}`, 'info', {
      status,
      timestamp: Date.now(),
      element: wsStatusIndicator ? 'found' : 'not found'
    });
  ...
  updateMQTTStatus(status) {
    ...
    // Enhanced debug logging for MQTT
    this.safeDebugLog(`MQTT Status Update: ${status}`, 'mqtt', {
      status,
      timestamp: Date.now(),
      element: mqttStatusIndicator ? 'found' : 'not found',
      previousClasses: mqttStatusIndicator ? Array.from(mqttStatusIndicator.classList) : []
    });
    ...
    switch (status) {
      case 'connected':
        ...
        this.safeDebugLog('MQTT connection established successfully', 'mqtt', { broker: 'TLS WebSocket' });
        break;
      case 'connecting':
        mqttStatusIndicator.classList.add('connecting');
        mqttStatusText.textContent = 'MQTT: Connecting...';
        this.safeDebugLog('MQTT attempting connection via WebSocket TLS', 'mqtt');
        break;
      case 'disconnected':
        mqttStatusIndicator.classList.add('disconnected');
        mqttStatusText.textContent = 'MQTT: Disconnected';
        this.safeDebugLog('MQTT connection lost', 'warn', { reason: 'Disconnected' });
        break;
      case 'error':
        mqttStatusIndicator.classList.add('error');
        mqttStatusText.textContent = 'MQTT: Error';
        this.safeDebugLog('MQTT connection error detected', 'error', { requiresInvestigation: true });
        break;
      case 'failed':
        mqttStatusIndicator.classList.add('failed');
        mqttStatusText.textContent = 'MQTT: Failed';
        this.safeDebugLog('MQTT connection failed completely', 'error', { retryRecommended: true });
        break;
      case 'offline':
        mqttStatusIndicator.classList.add('offline');
        mqttStatusText.textContent = 'MQTT: Offline';
        this.safeDebugLog('MQTT broker appears offline', 'error', { checkNetworkConnectivity: true });
        break;
      case 'unavailable':
        mqttStatusIndicator.classList.add('unavailable');
        mqttStatusText.textContent = 'MQTT: Broker Unavailable';
        this.safeDebugLog('MQTT broker unavailable', 'error', { checkBrokerStatus: true });
        break;
    }
  ...
  updateOverallConnectionStatus() {
    ...
    // Debug logging for connection status chain
    this.safeDebugLog(`Overall Connection Status: ${overallStatus}`, 'info', {
