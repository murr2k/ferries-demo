...
class MQTTClient {
  ...
  loadConfig(overrides = {}) {
    try {
      ...
      const activeProvider = process.env.MQTT_PROVIDER || defaultConfig.activeProvider || 'mosquitto';
      
      // Override with environment variables based on active provider
      if (activeProvider === 'hivemq') {
        if (process.env.HIVEMQ_CLUSTER_URL) {
          defaultConfig.providers.hivemq.broker.url = process.env.HIVEMQ_CLUSTER_URL;
        }
        if (process.env.HIVEMQ_USERNAME) {
          defaultConfig.providers.hivemq.authentication.username = process.env.HIVEMQ_USERNAME;
        }
        if (process.env.HIVEMQ_PASSWORD) {
          defaultConfig.providers.hivemq.authentication.password = process.env.HIVEMQ_PASSWORD;
        }
      } else if (activeProvider === 'mosquitto') {
        if (process.env.MQTT_BROKER_HOST) {
          defaultConfig.providers.mosquitto.broker.url = process.env.MQTT_BROKER_HOST;
        }
        if (process.env.MQTT_USERNAME) {
          defaultConfig.providers.mosquitto.authentication.username = process.env.MQTT_USERNAME;
        }
        if (process.env.MQTT_PASSWORD) {
          defaultConfig.providers.mosquitto.authentication.password = process.env.MQTT_PASSWORD;
        }
      }
      
      defaultConfig.activeProvider = activeProvider;
  ...
  getDefaultConfig() {
    return {
      ...
      providers: {
        hivemq: {
          ...
          authentication: {
            ...
            clientIdPrefix: 'bc-ferries-hivemq'
          },
          options: {
            keepalive: 60,
            clean: true,
        ...
        mosquitto: {
          broker: {
            url: 'bc-ferries-mqtt-broker.fly.dev',
            port: 443,
            protocol: 'wss',
            secure: true
          },
          authentication: {
            ...
            clientIdPrefix: 'bc-ferries-mqtt'
          },
          options: {
            keepalive: 60,
            clean: true,
      ...
      topicMappings: {
        ...
        mosquitto: {
          telemetry: {
            pattern: 'ferry/vessel/{vesselId}/telemetry',
            qos: 1
          },
          emergency: {
            pattern: 'ferry/vessel/{vesselId}/emergency/{type}',
            qos: 2
          }
        }
      },
  ...
  setupClient() {
    ...
    const providerConfig = this.config.providers[activeProvider];
    
    if (!providerConfig) {
      throw new Error(`Invalid MQTT provider: ${activeProvider}`);
    }
    
    const { broker, authentication, options } = providerConfig;
    ...
    if (authentication.username && authentication.password) {
      connectOptions.username = authentication.username;
      connectOptions.password = authentication.password;
    }
    
    // For WebSocket connections, add transformation function
    if (broker.protocol === 'wss' || broker.protocol === 'ws') {
      connectOptions.transformWsUrl = (url, options, client) => {
        // Ensure proper WebSocket URL format for Mosquitto
        console.log('🔧 WebSocket URL transformation:', url);
        return url;
      };
    }

    console.log(`🔗 Connecting to ${activeProvider.toUpperCase()} MQTT broker: ${brokerUrl}`);
  ...
  setupEventHandlers() {
    this.client.on('connect', () => {
      console.log(`✅ Successfully connected to ${this.activeProvider.toUpperCase()} MQTT broker`);
  ...
  publishTelemetry(vesselId, telemetryData) {
    const topicConfig = this.config.topicMappings[this.activeProvider].telemetry;
    const topic = topicConfig.pattern.replace('{vesselId}', vesselId);
    const qos = topicConfig.qos;
    
    const message = {
      ...telemetryData,
      messageId: telemetryData.messageId || uuidv4()
    };
  ...
  publishEmergency(vesselId, emergencyType, emergencyData) {
    const topicConfig = this.config.topicMappings[this.activeProvider].emergency;
    const topic = topicConfig.pattern
  ...
  publishStatus(vesselId, component, statusData) {
    const topicConfig = this.config.topicMappings[this.activeProvider].status;
    const topic = topicConfig.pattern
      .replace('{vesselId}', vesselId)
      .replace('{component}', component);
    const qos = topicConfig.qos;
    const retain = topicConfig.retain;
  ...
  startHeartbeat() {
    const heartbeatConfig = this.config.topicMappings[this.activeProvider].heartbeat;
  ...
  getConnectionInfo() {
    ...
    return {
      ...
      broker: `${providerConfig.broker.protocol}://${providerConfig.broker.url}:${providerConfig.broker.port}`
    };
  }
